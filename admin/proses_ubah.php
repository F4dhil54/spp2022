...
if ($_POST) {
    if ($_POST['siswa']) {
        ...
        if (empty($nama_petugas)) {
            ...
            if (empty($password)) {
                include "koneksi.php";
                $update = mysqli_query($conn, "update petugas set  nama_petugas='" . $nama_petugas . "', username='" . $username . "', level='" . $level . "'  where id_petugas = '" . $id . "' ") or die(mysqli_error($conn));
                if ($update) {
                ...
                include "koneksi.php";
                $update = mysqli_query($conn, "update petugas set  nama_petugas='" . $nama_petugas . "', username='" . $username . "', level='" . $level . "', password='" . md5($password) . "'  where id_petugas = '" . $id . "' ") or die(mysqli_error($conn));
                if ($update) {
                    echo "<script>alert('Sukses update petugas');location.href='datapetugas.php';</script>";
                } else {
                    echo "<script>alert('Gagal update petugas');location.href='ubah_petugas.php?id_petugas=" . $id . "';</script>";
